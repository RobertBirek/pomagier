...
export function LocationPicker({ onSelect }: LocationPickerProps) {
  ...
  const aisles = useMemo(
    () =>
      [...new Set(locations.filter((l) => l.area === selected.area).map((l) => l.aisle))].sort(
        (a, b) => a - b,
      ),
  ...
  const racks = useMemo(
    () =>
      [
        ...new Set(
          locations
            .filter((l) => l.area === selected.area && l.aisle === selected.aisle)
            .map((l) => l.rack),
        ),
      ].sort((a, b) => a - b),
    [locations, selected.area, selected.aisle],
  );
      ...
      [
        ...new Set(
          locations
            .filter(
              (l) =>
                l.area === selected.area && l.aisle === selected.aisle && l.rack === selected.rack,
            )
            .map((l) => l.shelf),
        ),
      ].sort((a, b) => a - b),
    [locations, selected.area, selected.aisle, selected.rack],
  );
  ...
  const items =
    step === "area" ? areas : step === "aisle" ? aisles : step === "rack" ? racks : shelves;
