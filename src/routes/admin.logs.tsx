      ...
      {data?.rows && data.rows.length > 0 ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.rows.map((row) => (
                  <tr
                    key={row.id}
                    className={`border-b hover:bg-muted/30 ${row.type === "audit" ? "bg-muted/20" : ""}`}
                  >
                    {row.type === "audit" ? (
                      <>
                        <td colSpan={2} className="px-4 py-2">
                          ...
                          {row.details && (
                            <div className="text-xs text-muted-foreground mt-0.5 font-mono">
                              {(() => {
                                try {
                                  return JSON.stringify(JSON.parse(row.details));
                                } catch {
                                  return row.details;
                                }
                              })()}
                        ...
                        <td
                          colSpan={4}
                          className="px-4 py-2 text-right text-xs text-muted-foreground font-mono whitespace-nowrap"
                        >
          ...
          {data.total > 50 && (
            <div className="flex justify-center gap-2">
              <button
                ...
                disabled={page <= 1}
                className="rounded border px-3 py-1 text-sm hover:bg-accent disabled:opacity-30"
              >
                ←
              </button>
        ...
        !isLoading && (
          <EmptyState
            ...
            title="Brak wpisów"
            description="Ruchy będą widoczne po pierwszych operacjach na lokalizacjach"
          />
        )
      )}
    </div>
