...
import { Database, Download, Trash2, Play, Shield, HardDrive, Cloud } from "lucide-react";
import { useState, useEffect } from "react";
import { toast } from "sonner";

type TabKey = "config" | "backups" | "restore";
...
async function saveBackupConfig(data: {
  endpoint: string;
  bucket: string;
  region: string;
  accessKey: string;
...
async function testS3(data: {
  ...
  const r = await fetch("/api/backup/test-s3", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
  });
...
function AdminBackup() {
  ...
  const [s3TestResult, setS3TestResult] = useState<{ ok: boolean; error?: string } | null>(null);
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => toast.success("Konfiguracja zapisana"),
    onError: (e: Error) => toast.error(e.message),
  });
  const testS3Mut = useMutation({
    mutationFn: testS3,
    onSuccess: (d) => {
      setS3TestResult(d);
      if (d.ok) {
  ...
  const backupMut = useMutation({
    ...
    onSuccess: (d) => {
      toast.success(`Backup: ${d.filename}`);
      qc.invalidateQueries({ queryKey: ["backups"] });
    },
    onError: (e: Error) => toast.error(e.message),
  });
  const restoreMut = useMutation({
    mutationFn: () => restoreBackup(restoreFile),
    onSuccess: () => toast.success("Baza przywrócona — zrestartuj API"),
    onError: (e: Error) => toast.error(e.message),
  });
  const deleteMut = useMutation({
    mutationFn: ({ name, source }: { name: string; source: string }) => deleteBackup(name, source),
    onSuccess: () => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Automatyczny backup bazy danych, konfiguracji i certyfikatów
        </p>
      </div>
      ...
      <div className="flex gap-1 border-b">
        {tabs.map((t) => (
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 text-sm font-semibold border-b-2 transition-colors ${tab === t.key ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}
          >
      ...
      {tab === "config" && (
        <div className="max-w-xl space-y-4">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => testS3Mut.mutate(s3Form)}
          ...
          {s3TestResult && (
            <div
              className={`rounded p-3 text-sm ${s3TestResult.ok ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}
            >
              {s3TestResult.ok ? `✅ Połączono` : `❌ ${s3TestResult.error}`}
            </div>
          )}
        </div>
      )}
      ...
      {tab === "backups" && (
        <div className="space-y-4">
          ...
          >
            ...
            {backupMut.isPending ? "Wykonuję…" : "Wykonaj backup teraz"}
          </button>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/50">
                    <th className="px-4 py-2 text-left">Plik</th>
                    <th className="px-4 py-2 text-left">Źródło</th>
                    <th className="px-4 py-2 text-right hidden sm:table-cell">Rozmiar</th>
                  ...
                  {backups.map((b) => (
                    <tr key={b.name} className="border-b hover:bg-muted/30">
                      ...
                      <td className="px-4 py-2">
                        <StatusBadge tone={b.source === "s3" ? "info" : "success"}>
                          {b.source === "s3" ? (
                      ...
                      <td className="px-4 py-2">
                        <div className="flex gap-1">
                          <a
                            href={`/api/backup/download/${b.name}?source=${b.source}`}
                          ...
                          <button
                            onClick={() => {
                              if (confirm("Usunąć backup?"))
                                deleteMut.mutate({ name: b.name, source: b.source });
                            }}
                            className="touch-target rounded p-1 hover:bg-accent text-destructive"
                            title="Usuń"
                          >
      ...
      {tab === "restore" && (
        <div className="max-w-xl space-y-4">
          <SectionTitle
            ...
            description="Wybierz plik backupu do przywrócenia. Operacja nadpisuje obecną bazę danych."
          />
          <div className="rounded-lg border-2 border-destructive/30 bg-destructive/5 p-4">
            <div className="flex items-center gap-2 text-destructive font-semibold text-sm mb-2">
              ...
              UWAGA: Ta operacja nadpisze wszystkie dane!
            </div>
            <select
              value={restoreFile}
              onChange={(e) => setRestoreFile(e.target.value)}
              className="w-full rounded border bg-background px-3 py-2 text-sm font-mono mb-3"
            >
            ...
            <button
              ...
              disabled={restoreMut.isPending || !restoreFile || restoreConfirm !== "TAK"}
              className="w-full rounded-md bg-destructive py-2.5 text-sm font-medium text-destructive-foreground disabled:opacity-50"
            >
