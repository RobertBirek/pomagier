...
async function fetchLocationProducts(code: string) {
  ...
  return res.json() as Promise<{ id: number; symbol: string; name: string; barcode: string; unit: string }[]>;
}
...
function LocationCard() {
  ...
  const { data: products, isLoading, error } = useQuery({ queryKey: ["location", code], queryFn: () => fetchLocationProducts(code) });
    ...
    <div className="mx-auto max-w-md space-y-4 p-4">
      ...
      {parsed && (
        <div className="rounded-lg border bg-card p-4 space-y-2 text-sm">
          <div className="flex items-center gap-2"><Layers className="h-4 w-4 text-muted-foreground" /><span>Obszar {parsed.area}, Alejka {parsed.aisle}, Regał {parsed.rack}, Półka {parsed.shelf}</span></div>
        </div>
      )}
            ...
            {products.map((p) => (
              <div key={p.id} className="rounded-lg border bg-card p-3 flex items-center justify-between">
                <div className="min-w-0">
                  ...
                  {p.barcode && <div className="text-xs text-muted-foreground font-mono mt-0.5"><Barcode className="inline h-3 w-3" /> {p.barcode}</div>}
                </div>
      ...
      {products && products.length === 0 && !isLoading && (
        <div className="text-center py-8 text-muted-foreground"><Package className="mx-auto h-12 w-12 opacity-20 mb-2" /><p className="text-sm">Brak produktów</p></div>
      )}
    </div>
  );
}
