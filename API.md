...
## Stan: v1.6.2 — Produkcyjny (Sprint 4: global warehouses)

## Autoryzacja

- Sesja: cookie `token` (`HttpOnly`, `SameSite=Strict`, `Secure` w produkcji).
- Publiczne: `POST /api/login`, `GET /api/health`, `GET /api/wizard/status`, `/ca`, `/api/ca`, `/api/users`, `/api/warehouses`, `/api/erp-config`, `/api/test-connection`, `/api/wizard/*`, `/api/erp/supported-warehouses`.
...
| GET    | `/api/warehouses`                  | users.ts                       | Lista obsługiwanych magazynów (filtrowana)         |
| GET    | `/api/stats`                       | stats.ts                       | KPI (produkty, magazyny, użytkownicy)             |
| POST   | `/api/scan`                        | scan.ts                        | Skanuj kod EAN/symbol (body: `{code, warehouse?}`) |
| POST   | `/api/scan-basket`                 | scan.ts                        | Skan koszyka (body: `{code, warehouse?}`)         |
...
| GET    | `/api/erp/supported-warehouses`    | erp-supported-warehouses.ts    | Lista magazynów + włączone (admin)                |
| PUT    | `/api/erp/supported-warehouses`    | erp-supported-warehouses.ts    | Ustaw listę włączonych magazynów (admin)          |
